...
async function storeData() {
  ...
  let sak = document.getElementById("sak").files;
  let family = document.getElementById("family").files;
  let agency = document.getElementById("agency").files;
  ...
  if (storedData) {
    ...
    for (let i = 0; i < sak.length; i++) {
      let fileName = sak[i].name.toLowerCase();
      let fileExtension = fileName.split(".").pop();

      formData.append("sak", sak[i]);
    }
    for (let i = 0; i < family.length; i++) {
      let fileName = family[i].name.toLowerCase();
      let fileExtension = fileName.split(".").pop();

      formData.append("family", family[i]);
    }
    for (let i = 0; i < agency.length; i++) {
      let fileName = agency[i].name.toLowerCase();
      let fileExtension = fileName.split(".").pop();

      formData.append("agency", agency[i]);
    }
    ...
    try {
      const response = await axios.post(
        "https://inmaa.vercel.app/realEsate/add",
        formData,
        {
          headers: {
      ...
      if(response.data.message==="Data Added Successfully"){
        window.location.href = 'contract5.html';
    }
    else{
